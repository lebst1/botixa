...
async def start(message: types.Message):
    ...
    await message.answer(
        f"👋 Привет, {user_name}! Этот бот поможет тебе анонимно поделиться своими мыслями в канале.\n"
    )
...
# Проверяет наличие запрещенного контента в тексте:
#    - номера телефонов
#    - юзернеймы Telegram (@username)
#    - запрещенные слова

def has_forbidden_content(text: str) -> bool:
        
    if text and isinstance(text, str):
        text_without_spaces = text.replace(" ", "").lower() if text else ""

        if re.search(r" ?(карас|karas)[ьаеуиыояaeiouy] ?", text, re.IGNORECASE):
            return True

        if re.search(r" ?(/start|start)[ьаеуиыояaeiouy] ?", text, re.IGNORECASE):
            return True

        if re.search(r't\.me|telegram\.me', text):
            return True

        if re.search(r'\+?\d{10,15}', text):  # Поиск номеров телефонов
            return True

        if re.search(r'@\w+', text):  # Поиск юзернеймов Telegram
            return True
...
async def handle_message(message: types.Message, state: FSMContext):
    ...
    if message.text and has_forbidden_content(message.text):
        await message.reply("🚫 Извини, но отправка сообщений с таким содержанием запрещена.")
        return
...
#         try:
#             if message.from_user.id in ADMIN_ID_DELETE:
#                 await bot.delete_message(chat_id=chat_id, message_id=message_id)
#                 deleted_messages_count += 1
#                 await message.reply(f"Пост удален успешно.")
#             else:
#                 await message.reply("У вас нет прав на удаление сообщений.")
#         except Exception as e:
#             await message.reply(f"Ошибка при удалении поста: {e}")
#     else:
#         await message.reply("Это не пересланный пост. Перешлите мне пост из канала.")
# <----------------------МОДЕРАЦИЯ----------------------> #
...
async def handle_photo(message: types.Message):
    ...
    await bot.send_photo(CHAT_ID, photo_id, caption=caption, reply_markup=inline_keyboard)
...
@dp.message_handler(content_types=['text'], chat_id=CHANNEL_ID)
async def channel_message(message: types.Message):
    # Проверяем, является ли автор сообщения администратором
    if message.from_user.id in ADMINS:
        # Получаем юзернейм отправителя
        username = message.from_user.username
        # Отправляем уведомление с юзернеймом
        await message.answer(f"Админ @{username} опубликовал пост в канале!")
